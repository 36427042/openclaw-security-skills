...
def main():
    ...
    # 🔴 v3.3: Pipeline串行通信
    parser.add_argument('--chain', action='store_true', help='串联模式: 发布前自动调生菜(文案)+苦瓜(风控)')
    parser.add_argument('--input', type=str, default=None, help='从Pipeline上下文读取产品+文案')
    args = parser.parse_args()

    # 🔴 v3.3: Pipeline串联模式（生菜→苦瓜→发布）
    if args.chain or args.input:
        import subprocess as sp
        piped_file = args.input or os.environ.get("PIPELINE_CONTEXT_FILE",
                         os.path.expanduser("~/.openclaw/workspace/data/pipeline/context.json"))

        if args.chain:
            print("\n🔗 Pipeline串联模式: 生菜→苦瓜→发布")
            from datetime import datetime as dt
            ctx = {
                "chain": [], "products": [v[0] for v in ALL_PRODUCTS.values()],
                "step": 0, "started": dt.now().isoformat(),
            }
            os.makedirs(os.path.dirname(piped_file), exist_ok=True)
            with open(piped_file, 'w') as f:
                json.dump(ctx, f, ensure_ascii=False)
            # Step 1: 生菜文案
            print("  🥬 Step 1/3: 生菜·文案生成...")
            copy_script = os.path.expanduser("~/.openclaw/workspace/scripts/copy_engine.py")
            r1 = sp.run(["python3", copy_script, "--input", piped_file, "--output", piped_file],
                       capture_output=True, text=True, timeout=120)
            print(f"     {'✅' if r1.returncode == 0 else '❌'} {r1.stdout.strip()[-100:] if r1.stdout else ''}")
            # Step 2: 苦瓜风控
            print("  🥒 Step 2/3: 苦瓜·风控审核...")
            risk_script = os.path.expanduser("~/.openclaw/workspace/scripts/risk_controller.py")
            r2 = sp.run(["python3", risk_script],
                       capture_output=True, text=True, timeout=60,
                       env={**os.environ, "PIPELINE_CONTEXT_FILE": piped_file})
            print(f"     {'✅' if r2.returncode == 0 else '❌'} {r2.stdout.strip()[-80:] if r2.stdout else ''}")
            print("  🫘 Step 3/3: 豌豆·发布上架\n")
